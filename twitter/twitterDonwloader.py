	...
	def download(self):
		...
		if playlist.is_variant:
			...
			for plist in playlist.playlists:
				...
				#resolution_file = "C:/Users/ds/Downloads/media-scraper-master/" + resolution_file
...
def downloadImgs(url, type, folder):
	...
	if (type == 'video'):
		...
		twitter_dl = TwitterDownloader(url, folder, 0)
		twitter_dl.download()


	if (type == 'image'):
		patternGifUrlString = re.compile(r'https://pbs.twimg.com/tweet_video_thumb/.*.jpg')
		patternImgUrlString = re.compile(r'https://pbs.twimg.com/media/.*:')

		response = requests.get(url[:-8], headers={"X-Requested-With": "XMLHttpRequest"})

		mo = patternImgUrlString.findall(response.text)
		mo2 = patternGifUrlString.search(response.text)

		if mo != None:
			for fileUrl in mo:
				fileUrl = fileUrl.replace(":","")
				fileUrl = fileUrl.replace("https","https:")
				downFileName = fileUrl[fileUrl.rfind("/")+1:]
				print(folder+downFileName)
				urllib.request.urlretrieve(fileUrl, folder+downFileName)

		if mo2 != None:
			fileUrl = mo2.group().replace("https://pbs.twimg.com/tweet_video_thumb/", "")
			print(fileUrl)
			fileUrl = fileUrl.replace('.jpg', '')
			print(fileUrl)
			fileUrl = "https://video.twimg.com/tweet_video/"+fileUrl+".mp4"
			print(fileUrl)
			downFileName = fileUrl[fileUrl.rfind("/")+1:]
			print(downFileName)
			urllib.request.urlretrieve(fileUrl, folder+downFileName)
...
if __name__ == '__main__':
	...
	tweetFileName = "C:/tweets/"+username+".txt"
	now = datetime.datetime.now()
	nowDatetime = now.strftime('%Y-%m-%d %H-%M-%S')

	print(os.path.isfile(tweetFileName))
	if os.path.isfile(tweetFileName) :
		changeTweetFileName = "C:/tweets/"+username+" "+nowDatetime+".txt"
		os.rename(tweetFileName, changeTweetFileName)

	searchParameters = twint.Config()
	searchParameters.Username = username
	searchParameters.Format = "Post ID: {id} | Username: {username} | Date: {date} | Tweet: {tweet}"
	searchParameters.Output = "C:\\tweets\\" + username + ".txt"
	searchParameters.Store_object = True

	twint.run.Search(searchParameters)
	twint.run.get_event_loop()

	print(os.path.isfile(tweetFileName))

	folder = "C:/tweets/" + username + "/"
	r = open(tweetFileName, mode='rt', encoding='utf-8')

	pattern_string = re.compile(r'pic.twitter.com/[a-zA-Z0-9]*')
	patternVideoUrlString = re.compile(r'https://twitter.com/[a-zA-Z0-9_]*/status/[a-zA-Z0-9]*/video/[a-zA-Z0-9]*')
	patternImageUrlString = re.compile(r'https://twitter.com/[a-zA-Z0-9_]*/status/[a-zA-Z0-9]*/photo/[a-zA-Z0-9]*')

	while True:
		line = r.readline()
		if not line:
			break
		mo = pattern_string.search(line)
		if mo != None:
			#print(mo.group())
			response = requests.get('https://' + mo.group(),allow_redirects=False)
			print('url : ' + response.headers['location'])
			videoPattern = patternVideoUrlString.search(response.headers['location'])
			if videoPattern != None:
				#print(videoPattern)
				downloadImgs(videoPattern.group(), "video", folder)
			imagePattern = patternImageUrlString.search(response.headers['location'])
			if imagePattern != None:
				#print(imagePattern)
				downloadImgs(imagePattern.group(), "image", folder)
